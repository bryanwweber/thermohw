...
def process(exam_num: int, time: str, date: str) -> None:
    ...
    fw.write(combine_pdf_as_bytes(solution_pdfs), resources, f'exam-{exam_num}-{time}-soln')


def main(argv: Optional[Sequence[str]] = None) -> None:
    """Parse arguments and process the exam assignment."""
    parser = ArgumentParser(
        description="Convert Jupyter Notebook exams to PDFs",
    )
    parser.add_argument('--exam', type=int, required=True, help="Exam number to convert",
                        dest='exam_num')
    parser.add_argument('--time', type=str, required=True, help="Time of exam to convert")
    parser.add_argument('--date', type=str, required=True, help="The date the exam will take place")
